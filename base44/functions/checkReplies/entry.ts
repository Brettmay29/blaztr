        ...
        try {
              ...
              for (const account of gmailAccounts) {
                      ...
                      const accountCampaigns = campaigns.filter(
                                (c) => c.gmail_account_id === account.id && (c.status === 'Active' || c.status === 'Paused')
                              );
                      ...
                      for (const msg of messages) {
                                ...
                                const emailReceivedAt = parseInt(msgData.internalDate || '0');const sentEmails = new Set(sendLogs.map((l) => l.lead_email?toLowerCase()));
    ...
    for (const account of gmailAccounts) {
      ...
      for (const msg of messages) {
        ...
        if (matchingLog?.lead_id) {
          ...
          // FIX: Clear pending future steps so no more follow-ups fire after a reply
